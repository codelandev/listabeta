class Startup < ActiveRecord::Base
  ...
  scope :highlighteds, -> { where(highlighted: true, status: Status::APPROVED).order(:created_at) }
  scope :unhighlighteds, -> { where(highlighted: false, status: Status::APPROVED).order(:created_at) }
  scope :approvateds, -> { where(status: Status::APPROVED).order(:created_at) }
  scope :unapprovateds, -> { where(status: Status::UNAPPROVED).order(:created_at) }
